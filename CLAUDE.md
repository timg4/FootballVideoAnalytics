...
1. **`src/detect_track.py`** — YOLO-Personenerkennung + ByteTrack.
   ...
   **Wichtig:** `--imgsz 1280` + yolo11s ist Pflicht, sonst fehlen ferne Spieler.
   Frame-Indizes in der CSV sind absolut (Video-Frames), auch bei `--start` —
   team_assign richtet sich automatisch danach.
...
3. **`src/team_assign.py`** — Teams über Trikotfarben + Platz-Filter in einem:
   ...
   Aussortierte Personen werden im Video standardmäßig ausgeblendet;
   `--zeige-ignorierte` blendet sie grau ein (Kontrolle des Platz-Filters).
...
- **Kalibrierung gilt pro Kamera-Standort.** `data/calibration/platz_vorlaeufig.json`
  = Kamera der Highlight-Clips (31.05.). Von Claude per Median-Panorama abgelesen
...
- `data/videos/Video Project.mp4` — **14 min** am Stück (anderes Spiel, gleiche
  ...
  Das ist das nächste Analyse-Ziel. Smoke-Test bestanden: Erkennung 19-26
  Spieler/Frame, Teams = Grün-Leibchen vs. Blau-Shirts sauber getrennt
  (Test-Slice ab Sekunde 30, `data/output/projektvideo_test_*`).
...
- `register_frames.py`/`pitch_map.py` erwarten noch slice-relative NPZ-Indizes —
  bei Ausschnitten mit `--start` müssen Registrierung und Tracking denselben
  Ausschnitt meinen (Registrierung braucht dafür noch eine `--start`-Option).
